...
export default function Saved({ 
  ...
  useEffect(() => {
    async function loadSaved() {
      ...
      const filtered = all.filter(d => bookmarks.includes(d.id));
  ...
  if (isLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-3 bg-brand-lavender-tint">
        ...
        <p className="text-[10px] font-mono uppercase tracking-wider text-brand-purple/75">Retrieving saved actions database...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-3xl mx-auto text-brand-near-black">
      ...
      <div className="border-b border-brand-lavender/30 pb-5">
        <h1 className="font-fredoka font-bold text-2xl tracking-tight text-brand-purple">Saved Recommended Actions</h1>
        <p className="font-display text-[11px] uppercase tracking-wider text-brand-near-black/60 mt-1">
          Review operational recommendations that you bookmarked for reference or team meetings.
        </p>
      </div>

      {savedDecisions.length === 0 ? (
        <div className="bg-brand-white rounded-2xl border border-brand-lavender/30 p-12 text-center space-y-4 shadow-xs">
          <Bookmark className="w-12 h-12 text-brand-purple mx-auto fill-current animate-pulse" />
          <div className="space-y-2">
            <h3 className="font-display font-extrabold text-base uppercase text-brand-near-black">No Saved Actions</h3>
            <p className="text-xs text-brand-near-black/70 max-w-xs mx-auto leading-relaxed">
              When reviewing recommended actions, toggle the bookmark icon to save high-priority guidelines for later meetings.
            </p>
          </div>
          <button
            id="saved-start-explore-btn"
